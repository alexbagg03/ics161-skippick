...
[ExecuteInEditMode]
public class NestedPrefabInstance : MonoBehaviour
{
    ...
    void Rebuild(GameObject source, Matrix4x4 inMatrix)
    {
        ...
        foreach (NestedPrefabInstance pi in source.GetComponentsInChildren(typeof(NestedPrefabInstance), true))
        {
    ...
    {
        foreach (NestedPrefabInstance pi in UnityEngine.Object.FindObjectsOfType(typeof(NestedPrefabInstance)))
            BakeInstance(pi);
    }

    public static void BakeInstance(NestedPrefabInstance pi)
    {
        ...
        pi.prefab = null;
        foreach (NestedPrefabInstance childPi in go.GetComponentsInChildren<NestedPrefabInstance>())
